...
const MAX_AUTH_FILES_INLINE_METADATA_DOWNLOADS = 100;

const enrichAuthFilesWithPlanMetadata = async (
  payload: AuthFilesResponse
): Promise<AuthFilesResponse> => {
  const filesMissingMetadata = payload.files.filter((entry) => {
    if (hasAuthFilePlanMetadata(entry) || isRuntimeOnlyEntry(entry)) return false;
    return Boolean(readTextField(entry, 'name'));
  });

  if (filesMissingMetadata.length === 0) return payload;

  if (filesMissingMetadata.length > MAX_AUTH_FILES_INLINE_METADATA_DOWNLOADS) {
    return payload;
  }

  const metadataByName = new Map(
    await Promise.all(
      filesMissingMetadata.map(async (entry) => {
        const name = readTextField(entry, 'name');
        const json = await authFilesApi.downloadJsonObject(name);
        return [name, pickAuthFilePlanMetadata(json)] as const;
      })
    )
  );
